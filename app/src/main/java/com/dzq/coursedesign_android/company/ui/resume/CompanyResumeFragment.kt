package com.dzq.coursedesign_android.company.ui.resume
...
import com.dzq.coursedesign_android.databinding.FragmentCompanyResumeBinding
...
class CompanyResumeFragment : Fragment() {
    ...
    private var _binding: FragmentCompanyResumeBinding? = null

    // This property is only valid between onCreateView and
    // onDestroyView.
    private val binding get() = _binding!!

    private val companyResumeHandler = object : Handler(Looper.getMainLooper()) {
        override fun handleMessage(msg: android.os.Message) {
            ...
            when (msg.what) {
                200 -> {
                    ...
                    val recyclerView = binding.rvCompanyResume
                    recyclerView.layoutManager = LinearLayoutManager(context)
                    recyclerView.adapter = CompanyResumeAdapter(resumeRecordList)
                }
                else -> {
                    val result = msg.obj as Result
    ...
    override fun onCreateView(
        ...
        _binding = FragmentCompanyResumeBinding.inflate(inflater, container, false)
        val root: View = binding.root
        getResumeByCompanyId()

        return root
    }

    private fun getResumeByCompanyId() {
        context?.getSharedPreferences("company_user", Context.MODE_PRIVATE)?.apply {
            ...
            ResumeHttp.getCompanyResumeList(companyUser.companyId!!, companyResumeHandler)
        }
    }
